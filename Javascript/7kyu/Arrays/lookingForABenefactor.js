...
//PARAMETERS: we receive "arr" as an array of positive numbers INT or float. And "newavg" as a positive INT.
//RETURN: we must return a positive int number that represent how much we need to sum to "arr" to make a new average of them that goes with "newavg"
//EXAMPLE:
/*
...
//PSEUDOCODE:
/*
-We declare a new variable that holds the "total value".
-total value is made by multiplying "newavg" by lenght plus 1 minus the total sum of "arr"
-We check if total is less or equal to 0 and throw and error, else we return total rounded.

*/

function newAvg(arr, newavg){
    const total = newavg * (arr.length + 1) - arr.reduce((curr, prev) => curr + prev,0)
    if(total <= 0) {
